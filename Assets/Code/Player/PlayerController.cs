...
{
    // Notes: 
    //  Create State Machine and remove booleans. Deprecate Ctrl&Delegation.
    ...
    public float grindSpeedMult = 0.7f; // Speed multiplier for grinds.
    public float maxYVelocity; // Airborne velocity.
    public float jumpHeight = 3f; 
    public float flipSpeed = 170f; // Deg/sec
    public float autoRotateSpeed = 90f; // Deg/sec
    ...
    private void CompleteJump () { // This is used to reset hasJumped bool to false, and to finish any trick chains we've made while airborne.
    ...
    private void StickToGround () { // Adds force to make player stick to ground (When not jumping)
        if(!isGrounded)
            return;
        if(hasJumped)
            return;    
    ...
    private void FaceGround () { // While grounded we want player to align with terrain's normal at all times
    ...
    private bool AttemptRopeGrind (Collider2D col) {
        if(hasCrashed)
            return false;
    ...
    */
    private void RaysToGround () { // RAYCAST TO TERRAIN - Check for terrain's normal and see if player is grounded (Touching the terrain, or close to)
        int layerMaskA = 1 << 3;
        int layerMaskB = 1 << 14;
    ...
    void OnTriggerEnter2D(Collider2D col) {
        if(col.gameObject.layer == 11 && !col.isTrigger) { // 11 -> Rope grind
            lastRopeCol = col;
        }
    }
    void OnTriggerExit2D(Collider2D col) {
        if(col.gameObject.layer == 3) { // 3 -> Terrain
            isGrounded = false;
            hasJumped = true;
        }
        else if(col.gameObject.layer == 11 && !col.isTrigger) { // 11 -> Rope grind
            lastRopeCol = null;
        }
    ...
    void OnCollisionEnter2D (Collision2D col) {
        ...
        else if(col.gameObject.layer == 11 && !col.collider.isTrigger) { // 11 -> Rope grind
            isOnRope = true;
            hasJumped = true;
            isGrinding = true;
    ...
    void OnCollisionExit2D (Collision2D col) { // 11 -> Rope grind
    ...
    private void SnowglideAudioControl() {
        if(inControl) {
            if(isGrounded && !isGrinding) {
